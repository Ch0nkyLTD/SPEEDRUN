...
void show_json_demo() {
  ...
  json out;
  out["foobar"] = 13;
  DEBUG_LOG("json: %s", out.dump().c_str());
  char val[] = "{\"foo\":123}";
  json data = json::parse(val);

  DEBUG_LOG("json: %s", data.dump().c_str());
}
...
*/
  ...
  auto dump = body.dump();
  DEBUG_LOG("JSON body: %s", dump.c_str());
  size_t req_size =
  ...
  DEBUG_LOG("Payload size: %llu", strlen(req_buffer));
